    ...
    <section class="section">
      ...
      ?>
        <div class="row">
          <div class="col-lg-12">
            ...
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">List Authors <a class="btn btn-sm btn-primary float-end" href="<?= $admin_url ?>authors/new">Create Author</a></h5>
                    ...
                    ?>
                        <tr>
                          ...
                          <td>
                            <?php
                            if ($list['status'] == 1) {
                              echo '<a class="btn btn-sm btn-success" href="'.$admin_url.'authors/status/'.$list['author_id'].'/0"><i class="ri-eye-fill"></i></a>';
                            } else {
                              echo '<a class="btn btn-sm btn-danger" href="'.$admin_url.'authors/status/'.$list['author_id'].'/1"><i class="ri-eye-off-fill"></i></a>';
                            }
                            ?>
                            <a class="btn btn-sm btn-warning" href="<?= $admin_url ?>authors/update/<?= $list['author_id'] ?>"><i class="ri-pencil-fill"></i></a>
                            <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteView<?= $list['author_id'] ?>">
                            <i class="ri-delete-bin-2-fill"></i>
                            </button>
                            <div class="modal fade" id="deleteView<?= $list['author_id'] ?>" tabindex="-1">
                              <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title"><?= $list['aurthor_name']; ?></h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                  </div>
                                  <div class="modal-body">
                                   <p>Do you want to delete <b><?= $list['aurthor_name']; ?></b> </p>
                                  </div>
                                  <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                    <a href="<?= $admin_url.'authors/delete/'.$list['author_id'] ?>" class="btn btn-danger">Yes Delete</a>
                                  </div>
                                </div>
                              </div>
                            </div><!-- End Large Modal-->
      ...
      <?php
        ...
        if (isset($_POST['newUser'])) {
          extract($_REQUEST);
          $sql = "INSERT INTO `author` (`aurthor_name`, `status`, `created_at`, `creator_id`) VALUES ('$aurthor_name', 1, '$dateTime', '$user_id')";
          if ($conn->query($sql)) {
            echo redirect($admin_url . 'authors/list');
          }
        }
        // end insert 
      ?>
            ...
            <div class="card">
              <div class="card-body">
                ...
                <form class="row g-3 needs-validation" novalidate method="post" action="<?= $current_url ?>">
                  <div class="col-md-4">
                    <label for="aurthor_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="aurthor_name" required name="aurthor_name">
                  </div>
      ...
      <?php
        ...
        if(isset($_POST['updateUser'])){
          extract($_REQUEST);
          $sql = "UPDATE `author` SET aurthor_name = '$aurthor_name', updated_at = '$dateTime' WHERE `author_id` = $id";
          $qry = $conn->query($sql);
          if ($qry) {
           echo redirect($admin_url."authors/list");
          }
            ...
            <div class="card">
              <div class="card-body">
                ...
                <form class="row g-3 needs-validation" novalidate method="post" action="<?= $current_url ?>">
                  <div class="col-md-4">
                    <label for="aurthor_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="aurthor_name" required name="aurthor_name" value="<?= $edit['aurthor_name'] ?>">
                  </div>
                  <div class="col-12">
      ...
      <?php
          ...
          extract($_REQUEST);
          $sql = "UPDATE `author` SET `status` = '$status' WHERE `author_id` = $id";
          $qry = $conn->query($sql);
          if ($qry) {
            $sql =  "UPDATE `books` SET `status` = '$status' WHERE `author_id` = $id";
            $qry = $conn->query($sql);
            if ($qry) {
            echo redirect($admin_url."authors/list");
            }
