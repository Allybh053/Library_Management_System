...
<body>
    ...
    <main id="main" class="main">
        <div class="pagetitle">
            ...
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">Library Activity</li>
                </ol>
            </nav>
        </div><!-- End Page Title -->
        <section class="section">
            ...
            if ($page == "list") {
                $sql = "SELECT * FROM `library_activity`";
                $qry = $conn->query($sql);
                $num = $qry->num_rows;
            ?>
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <div class="card">
                            <div class="card-body">
                                <h5 class="card-title">List Library Activity <a class="btn btn-sm btn-primary float-end" href="<?= $admin_url ?>libraryActivity/new">Add Activity</a></h5>
                                        ...
                                        ?>
                                                <tr>
                                                    ...
                                                    <td>
                                                        <?php
                                                        if ($list['status'] == 1) {
                                                            echo '<a class="btn btn-sm btn-success" href="' . $admin_url . 'libraryActivity/status/' . $list['id'] . '/0"><i class="ri-eye-fill"></i></a>';
                                                        } else {
                                                            echo '<a class="btn btn-sm btn-danger" href="' . $admin_url . 'libraryActivity/status/' . $list['id'] . '/1"><i class="ri-eye-off-fill"></i></a>';
                                                        }
                                                        ?>
                                                        

                                                        <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteView<?= $list['id'] ?>">
                                                            <i class="ri-delete-bin-2-fill"></i>
                                                        </button>
                                                        <div class="modal fade" id="deleteView<?= $list['id'] ?>" tabindex="-1">
                                                            <div class="modal-dialog modal-lg">
                                                                <div class="modal-content">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <p>Do you want to delete <b><?= $list['title']; ?></b> </p>
                                                                    </div>
                                                                    <div class="modal-footer">
                                                                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                                                        <a href="<?= $admin_url . 'libraryActivity/delete/' . $list['id'] ?>" class="btn btn-danger">Yes Delete</a>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div><!-- End Large Modal-->
            ...
            <?php
                ...
                if (isset($_POST['newLink'])) {
                    ...
                    $cover_upload = "../uploads/libraryActivity/" . $cover;
                    $cover = "uploads/libraryActivity/" . $cover;
                    if (move_uploaded_file($cover_temp, $cover_upload)) {
                            $title = $conn->real_escape_string($_POST['title']);
                            $description = $conn->real_escape_string($_POST['description']);
                            $sql = "INSERT INTO `library_activity` ( `path`,`title`, `description`, `status`, `created_at`, `creator_id`) VALUES ('$cover', '$title','$description', 1, '$dateTime', '$user_id')";
                            if ($conn->query($sql)) {
                                echo redirect($admin_url . 'libraryActivity/list');
                            }
                    }
                    // end cover image
                }
                        ...
                        <div class="card">
                            <div class="card-body">
                                ...
                                <form class="g-3 needs-validation" novalidate method="post" action="<?= $current_url ?>" enctype="multipart/form-data">
                                    ...
                                    <div class="col-12">
                                        <button class="btn btn-primary" name="newLink" type="submit">Add Activity</button>
                                    </div>
                                </form>
            ...
            <?php
                ...
                extract($_REQUEST);
                $sql = "UPDATE `library_activity` SET `status` = '$status' WHERE `id` = $id";
                $qry = $conn->query($sql);
                if ($qry) {
                    echo redirect($admin_url . "libraryActivity/list");
                }
                ...
                // echo '23423';
                extract($_REQUEST);
                $sql = "SELECT * FROM `library_activity` WHERE id = $id";
                $qry = $conn->query($sql);
                $num = $qry->num_rows;
                $delete_libraryActivity = $qry->fetch_assoc();
                unlink('../' . $delete_libraryActivity['path']);
                // echo '<img src="'.'../../../'.$delete_libraryActivity['path'].'">';
                $sql = "DELETE FROM `library_activity` WHERE `id` = $id";
                $qry = $conn->query($sql);
                if ($qry) {
                    echo redirect($admin_url . "libraryActivity/list");
                }
