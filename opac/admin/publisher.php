    ...
    <section class="section">
      ...
      ?>
        <div class="row">
          <div class="col-lg-12">
            ...
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">List Publishers <a class="btn btn-sm btn-primary float-end" href="<?= $admin_url ?>publishers/new">Create Publisher</a></h5>
                    ...
                    ?>
                        <tr>
                          ...
                          <td>
                            <?php
                            if ($list['status'] == 1) {
                              echo '<a class="btn btn-sm btn-success" href="'.$admin_url.'publishers/status/'.$list['publisher_id'].'/0"><i class="ri-eye-fill"></i></a>';
                            } else {
                              echo '<a class="btn btn-sm btn-danger" href="'.$admin_url.'publishers/status/'.$list['publisher_id'].'/1"><i class="ri-eye-off-fill"></i></a>';
                            }
                            ?>
                            <a class="btn btn-sm btn-warning" href="<?= $admin_url ?>publishers/update/<?= $list['publisher_id'] ?>"><i class="ri-pencil-fill"></i></a>
                            <button type="button" class="btn btn-sm btn-danger" data-bs-toggle="modal" data-bs-target="#deleteView<?= $list['publisher_id'] ?>">
                            <i class="ri-delete-bin-2-fill"></i>
                            </button>
                            <div class="modal fade" id="deleteView<?= $list['publisher_id'] ?>" tabindex="-1">
                              <div class="modal-dialog modal-lg">
                                <div class="modal-content">
                                  <div class="modal-header">
                                    <h5 class="modal-title"><?= $list['publisher_name']; ?></h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                  </div>
                                  <div class="modal-body">
                                   <p>Do you want to delete <b><?= $list['publisher_name']; ?></b> </p>
                                  </div>
                                  <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                    <a href="<?= $admin_url.'publishers/delete/'.$list['publisher_id'] ?>" class="btn btn-danger">Yes Delete</a>
                                  </div>
                                </div>
                              </div>
                            </div><!-- End Large Modal-->
      ...
      <?php
        ...
        if (isset($_POST['newUser'])) {
          extract($_REQUEST);
          $sql = "INSERT INTO `publisher` (`publisher_name`, `status`, `created_at`, `creator_id`) VALUES ('$publisher_name', 1, '$dateTime', '$user_id')";
          if ($conn->query($sql)) {
            echo redirect($admin_url . 'publishers/list');
          }
        }
        // end insert 
      ?>
        <div class="row">
          <div class="col-lg-6">
            <div class="card">
              <div class="card-body">
                <h5 class="card-title">New Publisher</h5>
                <!-- Custom Styled Validation -->
                <form class="row g-3 needs-validation" novalidate method="post" action="<?= $current_url ?>">
                  <div class="col-md-4">
                    <label for="publisher_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="publisher_name" required name="publisher_name">
                  </div>
      ...
      <?php
        ...
        if(isset($_POST['updateUser'])){
          extract($_REQUEST);
          $sql = "UPDATE `publisher` SET publisher_name = '$publisher_name', updated_at = '$dateTime' WHERE `publisher_id` = $id";
          $qry = $conn->query($sql);
          if ($qry) {
           echo redirect($admin_url."publishers/list");
          }
            ...
            <div class="card">
              <div class="card-body">
                ...
                <form class="row g-3 needs-validation" novalidate method="post" action="<?= $current_url ?>">
                  <div class="col-md-4">
                    <label for="publisher_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="publisher_name" required name="publisher_name" value="<?= $edit['publisher_name'] ?>">
                  </div>
                  <div class="col-12">
      ...
      <?php
          ...
          extract($_REQUEST);
          $sql = "UPDATE `publisher` SET `status` = '$status' WHERE `publisher_id` = $id";
          $qry = $conn->query($sql);
          if ($qry) {
            $sql =  "UPDATE `books` SET `status` = '$status' WHERE `publisher_id` = $id";
            $qry = $conn->query($sql);
            if ($qry) {
            echo redirect($admin_url."publishers/list");
            }
